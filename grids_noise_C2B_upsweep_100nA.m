...
% Read the data file for C2 100nA on Upsweep
fileName = "GRIDS_DIONE_CALIBRATION_12142022_2_B_100nA_U.xlsx";
...
hold on;
meanLine = xline(meanC2Current, "-r", {"\mu", "= " + num2str(meanC2Current) + " nA"});
meanLine.LabelOrientation = "horizontal";
meanLine.FontWeight = 'bold';
hold on;
plusOneStdDev = xline(meanC2Current + stdC2Current, "-b", {"\mu + \sigma", "= " + num2str(meanC2Current + stdC2Current) + " nA"});
plusOneStdDev.LabelOrientation = "horizontal";
plusOneStdDev.FontWeight = 'bold';
hold on;
minusOneStdDev = xline(meanC2Current - stdC2Current, "-b", {"\mu - \sigma", "= " + num2str(meanC2Current - stdC2Current) + " nA"});
...
xlabel("Current (nA) on Collector B");
ylabel("Number of Measurements");
title("Distribution of Collector B Currents with 100 nA Stimulation");
subtitle("\mu = " + num2str(meanC2Current) + " nA, \sigma = " + num2str(stdC2Current) + " nA");
...
set(gca, 'XMinorGrid', 'on', 'YMinorGrid', 'on', ...
         'GridColor', 'k', 'MinorGridAlpha', 0.4, 'MinorGridLineStyle', '-', ...
         'MinorGridColor', [0.3,0.3,0.3], 'GridAlpha', 0.4, 'LineWidth', 1.2);
title("QQ Plot of Collector B Current with 100 nA Stimulation");
ylabel("Collector B current (nA)");
...
set(gca, 'XMinorGrid', 'on', 'YMinorGrid', 'on', ...
         'GridColor', 'k', 'MinorGridAlpha', 0.4, 'MinorGridLineStyle', '-', ...
         'MinorGridColor', [0.3,0.3,0.3], 'GridAlpha', 0.4, 'LineWidth', 1.2);
title("Moving Average of Collectors A, C, and D Currents for 100 nA Stimulation on Collector B");
subtitle("n = " + num2str((1/10)*size(currentC2_nA, 1)));
xlabel("Time (s)");
ylabel("Moving Average Current (nA)");
legend("Collector A Current", "Collector C Current", "Collector D Current");
xlim([0 numSeconds]);
hold off;
